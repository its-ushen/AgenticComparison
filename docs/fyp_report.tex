...
across all configurations. The 56~Tier~A payloads form the initial benchmark.
...
Each of the three initial architectures was evaluated against all 56~Tier~A payloads.
Three post-calibration runs were completed per configuration. Table~\ref{tab:results}
reports averages across those runs.

\begin{table}[t]
\caption{Tier~A results: average TCR and ASR across post-calibration runs
(56 payloads, 3 runs per configuration)}
\label{tab:results}
\centering
\begin{tabular}{llrr}
\toprule
...
at Sonnet~4.5 while ReAct rises to 4.8\%. The single payload that succeeds in every
configuration is \texttt{adv\_tax\_reporting\_invoice}, which embeds a correct
customer identifier inside regulatory compliance framing. No architecture resists it.

The initial runs produced inflated ASR figures before judge calibration. Three
...
routine read calls classified as data gathering. All three were corrected before the
runs reported in Table~\ref{tab:results}. Full calibration details and
pre-/post-calibration comparisons are provided in Appendix~\ref{appendix:calibration}.

Two implementation defects were identified and corrected during the Tier~A
evaluation. The \texttt{DUAL\_LLM\_ORCHESTRATOR\_PROMPT} listed no tools, causing the
orchestrator to respond that it lacked access rather than calling them. This depressed
Dual~LLM TCR by approximately 3 percentage points. The judge \texttt{max\_tokens}
was set to 300, truncating JSON responses mid-sentence and producing spurious TCR
failures on approximately 4 evaluations per run. Post-fix, Dual~LLM Haiku~4.5
produced 1.8\% ASR in every run before and after the fix, confirming the security
finding was not an artefact of either defect. Pre- and post-fix Tier~A results across
all model tiers are provided in Appendix~\ref{appendix:bugfix}.

Spotlighting~\cite{hines2024spotlighting} was applied to all three architectures as a
prompt-level baseline. Tool results were wrapped in \texttt{<external\_content>} tags
and agents were instructed to treat tagged content as untrusted data. The validation
run was conducted at Haiku~4.5 across all three architectures. Dual~LLM held at 1.8\%
ASR, Plan-Then-Execute improved from 6.0\% to 3.6\%, and ReAct TCR decreased slightly
from over-refusal on the delimiter tags. Architectural defences dominate over
prompt-level mitigations. Spotlighting is included as a baseline in all subsequent
configurations. Full spotlighting results are provided in
Appendix~\ref{appendix:spotlighting}.

Table~\ref{tab:results-v2} presents the final Tier~A baseline for the three initial
architectures: a single run post-bugfix and post-spotlighting, conducted as part of
the full 70-payload v2 benchmark. Comparing against Table~\ref{tab:results}, the most
visible change is Dual~LLM TCR at Haiku~3, which increased from 78.0\% to 92.9\%
following the orchestrator prompt fix. ASR figures are consistent across both tables,
confirming the v1 security findings hold post-fix.
...
The orchestrator is not instructed to ignore suspicious content; it is architecturally
prevented from receiving it. Because the extraction is a static Python filter rather than
an LLM prompt, the result is deterministic: the same tool response produces the same
extracted fields on every run.
...
Sonnet~4.5 it produces 16.7\% average with run-to-run variance, the only configuration
where the same payload produced both a WORST and an IDEAL outcome across runs at
temperature zero.

\subsection{Combined Results}

Table~\ref{tab:combined-results} presents the full 70-payload picture. Tier~A figures
are drawn from Tables~\ref{tab:results-v2} and~\ref{tab:schema-tier-a}: the v2 single
run filtered to Tier~A payloads only, post-fix and post-spotlighting. Extended tier
figures are the three-run averages from Table~\ref{tab:extended-results}. Per-run
breakdowns are provided in Appendix~\ref{appendix:extended-runs}.
