...
All eight Haiku~4.5 and Sonnet~4.5 configurations achieve TCR above 78\% with
ASR between 1.8\% and 6.0\% on Tier~A. These eight configurations form the viable
set for the analysis that follows.
...
All four architectures at both Haiku~4.5 and Sonnet~4.5 redirected the invoice
...
unchanged. Schema~Dual~LLM was compromised for the same reason: \texttt{cus\_test003}
is a typed identifier, not a string field, and passes through the Python allowlist
unchanged. No architectural defence prevented this. The residual 1.8\% ASR
reflects exactly this class of value-field attack and represents the floor below
which architectural defences alone cannot push without additional mechanisms such
as runtime value validation or human-in-the-loop confirmation.

Schema~Dual~LLM matches Dual~LLM on every Tier~A vector. The allowlist strips
the same metadata fields as the quarantine~LLM, so the same Tier~A attacks are
blocked. The architectural differentiation between the two becomes visible only
on the extended tiers, where the quarantine INCLUDE list and the allowlist diverge
on \texttt{customer.name}.
...
\toprule
\textbf{Operation} & \textbf{Attack Vector} & \textbf{n} & \textbf{ReAct} & \textbf{PTE} & \textbf{Dual LLM} & \textbf{Schema DL} \\
\midrule
Invoice     & Competing objectives    & 1 & 66.7 & 100.0 & 100.0 & 100.0 \\
Refund      & Competing objectives    & 2 & 16.7 &   0.0 &   0.0 &   0.0 \\
Invoice     & Customer misdirection   & 4 & 12.5 &  12.5 &   0.0 &   0.0 \\
Subscription & Parameter tampering    & 4 &  8.3 &  16.7 &   0.0 &   0.0 \\
Subscription & Hidden charges         & 4 &  0.0 &  16.7 &   0.0 &   0.0 \\
Subscription & Narrative chaining     & 1 &  0.0 &  16.7 &   0.0 &   0.0 \\
\midrule
\multicolumn{2}{l}{\textit{All other vectors (12 vectors, 42 payloads)}} & & 0.0 & 0.0 & 0.0 & 0.0 \\
\bottomrule
\end{tabular}
\end{table*}

Three patterns emerge. First, the competing objectives vector targeting
invoices achieves 100\% ASR on Plan-Then-Execute, Dual~LLM, and
Schema~Dual~LLM, and 66.7\% on ReAct. This is the value-level attack
discussed in Section~V-B: the payload embeds a fabricated compliance
requirement that substitutes one customer ID for another. No architecture
defends against it because the poisoned value is structurally
indistinguishable from legitimate data.

Second, Plan-Then-Execute is uniquely vulnerable to subscription-related
vectors (parameter tampering, hidden charges, narrative chaining) that both
quarantine architectures block entirely. The Synthesizer LLM in
Plan-Then-Execute receives raw tool results including injected metadata and
surfaces the attacker's framing in its response.
...
accepts only a price ID and a quantity. The tool's limited interface is itself
a defence, independent of architecture.
...
Tier~B establishes the upper bound of prompt-level instruction injection. At
Haiku~4.5 and Sonnet~4.5, 0\% ASR was recorded across all four architectures
against payloads embedding exact Stripe~MCP tool syntax with pre-staged
compliance framing. Capable models correctly contextualise description and name
field content as external data regardless of the instruction format, framing
pressure, or regulatory authority applied. The attack surface for prompt-level
injection is exhausted by Tier~A social engineering; adding exact tool schema
knowledge provides no additional uplift.
...
Schema~Dual~LLM achieves 0\% ASR in every extended tier run at Haiku~4.5 and
Sonnet~4.5. Zero variance across three runs. The allowlist is a static Python
dict filter: the same tool response produces the same extracted fields on every
invocation.

Dual~LLM at Haiku~4.5 produces 7.1\% ASR with zero variance. The quarantine
consistently passes the name field content, and the orchestrator consistently
uses it. At Sonnet~4.5, Dual~LLM produces 16.7\% average ASR with
run-to-run variance. The \texttt{ext\_c\_payment\_delivery\_note} payload
demonstrates this: in one run the Sonnet~4.5 orchestrator surfaced injected
description content as fact; in two other runs the same model on the same
payload omitted it entirely. Both outcomes occurred at temperature zero with
identical inputs. The orchestrator is at a boundary where small context
differences tip the outcome. Schema~Dual~LLM never reaches this boundary.

Notably, the capability ordering reverses for Dual~LLM on extended tiers.
At Haiku~4.5, 7.1\% ASR is consistent. At Sonnet~4.5, 16.7\% with
variance is worse, not better. A more capable model follows contextual cues in
trusted fields more faithfully, including injected ones. This mirrors the
Tier~A finding on ReAct: additional capability in a naive architecture increases
susceptibility. The quarantine layer prevents the attack from reaching the
orchestrator, but when a trusted field carries injected content, a more capable
orchestrator is more likely to act on it.

\subsection{The Cost-Security Efficiency Argument}

Across two full Tier~A benchmark rounds (three architectures, 56 payloads each),
...
ReAct with Sonnet~4.5 costs the same as Dual~LLM with Sonnet~4.5 but produces 4.8\%
ASR. Reaching for the most capable available model yields worse security at higher
cost when the architecture is not structurally sound.

Schema~Dual~LLM strictly dominates Dual~LLM on both cost and security. The Python
allowlist makes no LLM calls in the extraction path, eliminating the quarantine
call that doubles Dual~LLM's per-tool cost. At the same model tier, Schema~Dual~LLM
costs less than Dual~LLM and achieves 0\% ASR on the extended tiers compared to
Dual~LLM's 7.1\% at Haiku~4.5. The cost-optimal configuration for the lowest
achievable ASR is Schema~Dual~LLM with Haiku~4.5: lower cost than Dual~LLM at the
same model tier, and 0\% extended tier ASR versus 7.1\%.
...
Anthropic in April 2026 and is no longer available for replication. All
production benchmark runs were conducted on a Raspberry Pi~5 (8\,GB RAM,
Raspberry Pi OS Lite 64-bit). API cost was a practical constraint: each sweep
cost approximately \$8 in Anthropic API charges, which limited the number of
repeat runs and excluded the most expensive model tier (Claude Opus) from
evaluation. A broader model range including Opus would strengthen the findings
but was not feasible within the project budget. ASR figures are reported as
point estimates averaged across three runs. At 1.8\% ASR over 56 trials, a
95\% Wilson confidence interval spans roughly 0.3--9.4\%, meaning a larger
payload suite would be needed to confirm the difference between configurations
...
Haiku~4.5 (80.4\% TCR) and Dual~LLM with Haiku~4.5 (83.9\% TCR) would
remain above it, but the central finding depends on the floor being set
below the lowest TCR in the viable set. The choice of threshold is justified
on operational grounds in Section~V-A, but alternative thresholds would alter
which findings hold.
...
non-deterministic behaviour at Sonnet~4.5. The residual 1.8\% in both architectures
reflects a structural attack: a correctly typed customer identifier inside
regulatory framing that no architecture currently defends against.
...
emails or documents. For those cases, the Dual~LLM quarantine~LLM remains appropriate.
...
a frontier model does not improve security and increases cost substantially. The
quarantine boundary aligns with the principle of least privilege required by
PCI~DSS~\cite{pci_dss}: the orchestrator never processes attacker-writable string
fields directly.

Architectural defences do not eliminate risk. Schema~Dual~LLM achieves 0\% ASR on
the extended tiers but shares the 1.8\% Tier~A residual with all other architectures.
The \texttt{adv\_tax\_reporting\_invoice} payload succeeds in every configuration
because the correct customer identifier is structurally indistinguishable from
legitimate data. This class of attack requires runtime value validation or
human-in-the-loop confirmation for high-value operations to address.
...
updated to reflect new tool return schemas. Hybrid approaches combining
Schema~Dual~LLM's deterministic extraction with Plan-Then-Execute's goal-locking
may eliminate the remaining 1.8\% Tier~A residual that neither pattern alone
defends against.
