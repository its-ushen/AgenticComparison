...
Schema Dual LLM proposes to address the residual Tier~C vulnerability in Dual~LLM:
the quarantine passes certain trusted string fields to the orchestrator,
creating a probabilistic attack surface. The key observation is that the
quarantine LLM is doing something Python can do better for structured API
responses.

The quarantine LLM was designed for agents processing unstructured data:
emails, documents, and web pages where an LLM is needed to parse natural
language and extract structured fields. Stripe's MCP tools return
documented, fixed JSON. The schema is known in advance because the tools
were implemented against it. A quarantine LLM is unnecessary overhead for
this case and introduces its own attack surface: instruction text in a tool
result could influence the quarantine's extraction output.

Schema Dual LLM replaces the quarantine LLM with a Python allowlist. For
each Stripe tool, a hardcoded list of typed fields specifies exactly what
the orchestrator is permitted to see. The extraction is deterministic:
\texttt{description}, \texttt{name}, and \texttt{metadata} are absent from
every allowlist and never reach the orchestrator regardless of their
content. The allowlist for \texttt{customers.read} is
\texttt{[id,~email]}; \texttt{name} is absent. There is no LLM in the
extraction path.

This approach is directly inspired by CaMeL~\cite{camel2025}, which
addresses the same data flow problem using a Q-LLM with Pydantic schemas.
CaMeL uses an LLM because it is designed for agents processing unstructured
...
Dual~LLM with Haiku~4.5, Dual~LLM with Sonnet~4.5, and ReAct with Haiku~4.5
all recorded 1.8\% ASR with zero variance across three runs. Plan-Then-Execute
with Sonnet~4.5 showed the highest variance, ranging from 3.6\% to 7.3\% ASR
...
Dual~LLM matches Sonnet~4.5 at 1.8\% ASR. Sonnet~4.5 in ReAct (4.8\%) is worse
...
Pre-calibration, Sonnet~4.5 in the Dual~LLM architecture recorded 5.4--10.7\% ASR;
